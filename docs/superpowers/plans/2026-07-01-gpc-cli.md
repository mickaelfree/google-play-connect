...
skills/gpc-auth-setup/SKILL.md
skills/gpc-metadata-sync/SKILL.md
skills/gpc-release-flow/SKILL.md
...
Update semantics: fetch the existing listing inside the transaction (tolerating a 404 for a brand-new locale), then apply only the flags the user set, so an update of `--title` alone never blanks the descriptions.
...
Add `"io"` to the import list of this test file (used by `io.ReadAll`).
...
func newTracksCmd(deps Deps, flags *RootFlags) *cobra.Command {
	...
	for _, c := range []*cobra.Command{list, get} {
		c.Flags().StringVar(&app, "app", "", "package name of the app (required)")
		_ = c.MarkFlagRequired("app")
	}

	tracksCmd.AddCommand(list, get)
	return tracksCmd
}
```
...
```bash
git add internal/cmd/
git commit -m "feat: gpc tracks (list/get) and gpc releases publish (staged rollout, notes)"
```
	...
	statusCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, tr := range tracks {
				...
				if listErr != nil {
					// A track with no releases can 404; skip it silently only
					// when scanning all tracks, fail when it was explicit.
					if track != "" {
						return listErr
					}
					continue
...
- Produces:
  ...
  - `metadata.Issue{Locale, Field, Message string}`, `metadata.ValidateListing(locale string, l Listing) []Issue`, `metadata.ValidateTree(root, pkg string) ([]Issue, error)`
...
// ValidateTree validates every locale listing under root for the app.
...
**Files:**
- Create: `skills/gpc-auth-setup/SKILL.md`
- Create: `skills/gpc-metadata-sync/SKILL.md`
- Create: `skills/gpc-release-flow/SKILL.md`
...
- [ ] **Step 1: Write `skills/gpc-auth-setup/SKILL.md`**
...
- [ ] **Step 2: Write `skills/gpc-metadata-sync/SKILL.md`**
...
- [ ] **Step 3: Write `skills/gpc-release-flow/SKILL.md`**
...
- [ ] **Step 6: Write `internal/cmd/skills.go`**

`go:embed` cannot reach `../../skills` from `internal/cmd`, so the embed lives in a tiny package at the repo root. Create `skillsfs/skillsfs.go`:
...
Move the `skills/` directory handling: the embed directive requires `skillsfs.go` to sit NEXT TO the `skills/` dir. Place the file at the repo root as `skillsfs.go` with package name `skillsfs`? A root-level Go file must belong to the module's root package. Cleanest: create directory `skillsfs/` containing `skillsfs.go` AND move `skills/` inside it (`skillsfs/skills/...`). Adjust Steps 1–3 paths accordingly: the three SKILL.md files live at `skillsfs/skills/<name>/SKILL.md`.
...
- **Spec coverage:** apps (adjusted: `details`, no list — API limitation), edits, listings, images, tracks, releases, bundles, status, metadata pull/push/validate, 3 agent skills, install-skills, SETUP.md, Makefile/`go install` distribution — all covered by Tasks 1–15. Release notes moved from metadata push to `releases publish --notes-dir` (API-model deviation, documented in Task 11 and the spec).
